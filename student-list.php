...
include_once("includes/dbconnection.php");
  ...
  if(isset($_REQUEST['sid']) && $_REQUEST['act'] !=""){
    ...
    $query = "UPDATE students SET status='$sts' WHERE id =".$id;
    $delstudent= mysqli_query($conn,$query);
    if($delstudent){
        echo "<script>window.location='student-list.php'</script>";
    }
...
<div class="col-9"><h1>Student List</h1>   
  <div class="row">
    <div class="col-12" >
      <table class="table table-bordered" style="border-color: #000;background-color: #f8f9fa;">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><a class="btn btn-info" href="student-list.php?act=<?php echo $row['status']; ?>&sid=<?php echo $row['id']?>"><?php echo $row['status']; ?></a></td>      
            
            <td><a class="btn btn-info" href="edit-student-by-admin.php?id=<?php echo $row['id']?>">Edit</a></td>
            <td><a class="btn btn-danger" href="student-list.php?act=del&id=<?php echo $row['id']?>">Delete</a></td>
          </tr>
